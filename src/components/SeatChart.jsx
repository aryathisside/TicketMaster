...
const SeatChart = ({ occasion, ticketMasterContract, provider, setToggle }) => {

  const [seatsTaken, setSeatsTaken] = useState(false)
  const [hasSold, setHasSold] = useState(false)

  const getSeatsTaken = async () => {
    const seatsTaken = await ticketMasterContract.getSeatsTaken(occasion.id)
  ...
  const buyHandler = async (_seat) => {
    ...
    const transaction = await ticketMasterContract.connect(signer).mint(occasion.id, _seat, { value: occasion.cost })
